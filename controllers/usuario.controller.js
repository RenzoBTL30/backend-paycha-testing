...
export const createUsuario = async (req, res) => {
    ...
    try {
      pool.query(
        "INSERT INTO tb_usuario (email, nombre, apellidos, celular, contrasenia, estado) VALUES(?, ?, ?, ?, ?, '1');",
        [P_email, P_nombre, P_apellidos, P_celular, P_contrasenia, P_estado],
        function (err, result) {
          try {
...
export const listarUsuariosEmpresa = async (req, res) => {
  ...
  try {
    pool.query(
      "SELECT u.id_usuario, u.email, u.nombre, u.apellidos, u.celular, r.id_rol, r.nombre as rol, u.estado FROM tb_usuario_rol as ur JOIN tb_usuario as u ON ur.id_usuario = u.id_usuario JOIN tb_rol as r ON ur.id_rol = r.id_rol WHERE u.estado = 1 && r.nombre != 'Cliente';",
      function (err, result) {
        try {
